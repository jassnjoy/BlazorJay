...
namespace Core5_API.Controllers
{
	...
	public class DepartmentAPIController : ControllerBase
	{
		...
		[HttpPost("/GetDeptByName")]
		public async Task<IEnumerable<Department>> PostAsyncSearch(Department dept)
		{
			if (ModelState.IsValid)
			{
				var result = await deptServ.GetAsync(dept);//await deptServ.CreateAsync(dept);
				return result;
			}
			return null;
		}
